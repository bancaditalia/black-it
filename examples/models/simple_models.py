...
def NormalBer_3P(  # noqa: N802
    ...
    y = np.zeros(N)
    b = bernoulli.rvs(theta[0], size=N, random_state=rng)
    for i in range(N):
        if b[i] == 1:
            y[i] = rng.normal(theta[1], 0.1, 1)
        else:
            y[i] = rng.normal(theta[2], 0.1, 1)
...
def NormalBer_5P(  # noqa: N802
    ...
    for i in range(N):
        if b[i] == 1:
            y[i] = rng.normal(theta[1], theta[3], 1)
        else:
            y[i] = rng.normal(theta[2], theta[4], 1)
...
def Alpha(  # noqa: N802
    ...
    y = (alpha.rvs(theta[0], size=N, random_state=rng) + theta[1]) * theta[2]
...
def MarkovC_2P(  # noqa: N802
    ...
    for i in range(N):
        if s == 0:
            b = bernoulli.rvs(theta[0], size=1, random_state=rng)
            if b == 1:
                s = 1
                y[i] = rng.normal(x[1], sigma, 1)
            else:
                y[i] = rng.normal(x[0], sigma, 1)
        if s == 1:
            b = bernoulli.rvs(theta[1], size=1, random_state=rng)
            if b == 1:
                s = 0
                y[i] = rng.normal(x[0], sigma, 1)
            else:
...
def AR1(  # noqa: N802
    ...
    # Y_t = p0*Y_t-1 + eps
    y = np.zeros(N)
    y[0] = rng.normal(0, 1, 1)
    for i in range(1, N):
        y[i] = theta[0] * y[i - 1] + rng.normal(0, 1, 1)
...
def AR1_2P(  # noqa: N802
    ...
    y[0] = rng.normal(0, 1, 1)
    for i in range(1, N):
        y[i] = theta[0] * y[i - 1] + rng.normal(0, theta[1], 1)
    return np.atleast_2d(y).T
...
def AR1_3P(  # noqa: N802
    ...
    y[0] = rng.normal(0, 1, 1)
    for i in range(1, N):
        y[i] = theta[0] * y[i - 1] + rng.normal(theta[2], theta[1], 1)
...
def RWSB1(  # noqa: N802
    ...
    e = np.zeros(N)
    e[0 : theta[0]] = rng.normal(0, 0.1, theta[0])
    e[theta[0] :] = rng.normal(0, 0.2, N - theta[0])
...
def RWSB2(  # noqa: N802
    ...
    e = np.zeros(N)
    e[0:700] = rng.normal(0, theta[0], 700)
    e[700:] = rng.normal(0, theta[1], N - 700)
